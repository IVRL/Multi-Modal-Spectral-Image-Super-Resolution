...
def train(training_data_loader, optimizer, model, mrae, sid, epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        data, datatif, target = Variable(batch[0]).cuda(), Variable(batch[1]).cuda(), Variable(batch[2]).cuda()
        data, datatif, target = torch.clamp(data / 65535.0, 0, 1),torch.clamp(datatif / 65535.0, 0, 1), target / 65535.0
        output = torch.clamp(model(data, datatif), 0, 1)
        ...
        origin_mrae_loss = mrae(data, target, mask)
        origin_sid_loss = sid(data, target, mask)
        origin_loss = origin_mrae_loss + origin_sid_loss;
        ...
        if iteration % 50 == 0:
            print("===> Epoch[{}]({}/{}): Loss: {:.10f}/Origin{:.10f} + {:.10f}/Origin{:.10f} = {:.10f}/Origin{:.10f}".format(epoch, iteration, 
                len(training_data_loader), mrae_loss.item(), origin_mrae_loss.item(), sid_loss.item(), origin_sid_loss.item(), loss.item(), origin_loss.item()))
